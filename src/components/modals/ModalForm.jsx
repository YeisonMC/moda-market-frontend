...
import LogoBanner from "../../assets/svg/logobanner.svg";
import axios from "axios";

const ModalForm = ({ isVible, onClose }) => {
  if (!isVible) return null;
  ...
  const handleLogin = async (e) => {
    ...
    if (!email || !password) {
      return toast.info("Llena los campos");
    }
    ...
    try {
      const response = await axios.post(
        "https://moda-market-production.up.railway.app/modamarket/store/login",
        data
      );
      ...
      // por ejemplo, redirigir al usuario a otra página si la autenticación es exitosa
    } catch (error) {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex items-center justify-center">
        <div className="w-[600px] max-md:w-11/12 flex flex-col bg-white p-4 ">
          <div
            className="flex items-center place-self-end gap-2 cursor-pointer hover:bg-[#e54246] hover:text-white p-3 rounded-xl hover:font-semibold"
            onClick={() => onClose()}
          >
            <button className="text-base">Cerrar</button>
            <span>x</span>
          </div>
          <div className="mt-4">
            <p className="text-center">INICIA SESIÓN CON TU CUENTA</p>
            <img className="mx-auto mt-4" src={LogoBanner} alt="" />
            <div className="relative py-3 sm:max-w-xl sm:mx-auto">
              <div className="max-w-md mx-auto">
                ...
                <div className="mt-5">
                  <button
                    className="py-2 px-4 bg-[#e54246] hover:bg-red-700 focus:ring-red-500 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg"
                    type="submit"
                    onClick={handleLogin}
                  >
                    Iniciar sesión
